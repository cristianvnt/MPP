...
int main(int argc, char** argv)
{
    ...
    {
        if (rank == 0)
            std::cerr << "Use: mpirun -np <P> ./cannon <M> <FileA> <FileB> <FileC> [parallelRead]\n";
        MPI_Finalize();
        return 1;
    }
    ...
    bool parallelRead = (argc > 5 && std::strcmp(argv[5], "1") == 0);
    ...
    if (parallelRead)
    {
        ...
        for (size_t i = 0; i < blockSize; ++i)
        {
            ...
            infA.seekg(offset);
            infA.read(reinterpret_cast<char*>(localA.data() + i * blockSize),
                      blockSize * sizeof(double));

            infB.seekg(offset);
            infB.read(reinterpret_cast<char*>(localB.data() + i * blockSize),
                      blockSize * sizeof(double));
        }
    }
    else
